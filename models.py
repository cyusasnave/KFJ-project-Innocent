...
class JobSubCategory(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    category = Column(UUID(as_uuid=True),ForeignKey("job_categories.id",ondelete='CASCADE'))
...
class Job(Base):
    ...
    job_sub_category_id = Column(UUID(as_uuid=True),ForeignKey("job_sub_categories.id"))
