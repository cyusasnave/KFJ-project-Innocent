...
async def add_employer_profile(user_id:str,request:EmployerModel,current_user: Annotated[User, Depends(get_current_admin_user)],db: Session = Depends(get_db)):  
    user = db.query(User).filter(User.id == user_id and User.role == "employer").first()
    if  user is None:
        raise HTTPException(status_code=404, detail="User Not Found!")
    new_data = Employer(type_of_employer=request.type_of_employer,
                         user_id=user_id,#
                         name=request.name,
                         logo_url="",
                         contract_url="",
...
async def get_employer(current_user: Annotated[User, Depends(get_current_admin_user)],db: Session = Depends(get_db)):  
