...
from app.internal.models import JobCategory,JobSubCategory,JobRequest,User,Job,Employer
from app.internal.schemas import JobModel,JobViewModel,JobCategoryModel,JobCategoryView,SubJobCategoryModel,SubJobCategoryView
...
async def add_new_job(job_category_id:str,job_sub_category_id:str,request: JobModel,current_user: Annotated[User, Depends(get_current_employer)],db: Session = Depends(get_db)):
    category = db.query(JobCategory).filter(JobCategory.id == job_category_id).first()
    if category is None:
        raise HTTPException(status_code=404, detail="Category of job doesn't exists!")
    
    sub_category = db.query(JobSubCategory).filter(JobSubCategory.id == job_sub_category_id).first()
    if category is None:
        raise HTTPException(status_code=404, detail="Category of job doesn't exists!")
