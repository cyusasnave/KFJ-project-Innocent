...
async def add_employee_profile(
    ...
    user = db.query(Employee).filter(Employee.user_id == current_user.id).first()
    if user:
        user.user_specialization=specialization_id,
...
@router.post("/api/v1/employee/add/cv_file")
...
@router.post("/api/v1/employee/add/profile_picture")
async def add_employee_cv_picture(
    current_user: Annotated[User, Depends(get_current_employee)],
    db: Session = Depends(get_db),
    ...
    if employee is None:
        raise HTTPException(status_code=404, detail="Employee Not Found!")
    image_file_path = save_uploaded_picture(profile_picture)
    employee.profile_url = image_file_path
    db.commit()
    return "Profile picture uploaded SuccessFully!"


@router.post("/api/v1/specialization/add/")
...
@router.get("/api/v1/specialization/all", response_model=List[SpecializationView])
